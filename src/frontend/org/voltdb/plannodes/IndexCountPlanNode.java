...
public class IndexCountPlanNode extends AbstractScanPlanNode {
    ...
    private IndexCountPlanNode(IndexScanPlanNode isp,
            AggregatePlanNode apn,
            IndexLookupType endType,
            List<AbstractExpression> endKeys) {
    ...
    private void setSkipNullPredicate(boolean isReverseScan) {
        ...
        m_skipNullPredicate = IndexScanPlanNode.buildSkipNullPredicate(nextKeyIndex, m_catalogIndex, m_tableScan, m_searchKeyExpressions);
        if (m_skipNullPredicate != null) {
            m_skipNullPredicate.resolveForTable((Table)m_catalogIndex.getParent());
        }
    }

    // Create an IndexCountPlanNode that replaces the parent aggregate and
    // child indexscan UNLESS the indexscan's end expressions aren't a form
    // that can be modeled with an end key.
    // The supported forms for end expression are:
    //   - null
    //   - one filter expression per index key component (ANDed together) as
    //     "combined" for the IndexScan.
    //   - fewer filter expressions than index key components with one of them
    ...
    // causing undercounts.
    // A prefix-only LT filter is intended to discard prefix-equal cases,
    // so it is allowed.
    // @return the IndexCountPlanNode or null if one is not possible.
    public static IndexCountPlanNode createOrNull(IndexScanPlanNode isp,
            AggregatePlanNode apn) {
        // add support for reverse scan
        // for ASC scan, check endExpression;
        // for DESC scan, need to check searchKeys
        List<AbstractExpression> endKeys = new ArrayList<>();
        // Initially assume that there will be an equality filter
        ...
        for (AbstractExpression ae: endComparisons) {
            // There should be no more end expressions
            // after an LT or LTE has reset the end type.
            assert(endType == IndexLookupType.EQ);

            if (ae.getExpressionType() == ExpressionType.COMPARE_LESSTHAN) {
                endType = IndexLookupType.LT;
            }
            else if (ae.getExpressionType() ==
    ...
    public void toJSONString(JSONStringer stringer) throws JSONException {
        ...
        stringer.key(Members.END_TYPE).value(m_endType.toString());
        stringer.key(Members.TARGET_INDEX_NAME).value(m_targetIndexName);

        stringer.key(Members.ENDKEY_EXPRESSIONS);
    ...
    public void loadFromJSONObject(JSONObject jobj, Database db)
        ...
        m_endType = IndexLookupType.get(jobj.getString(Members.END_TYPE));
    ...
    protected String explainPlanForNode(String indent) {
        ...
        // Explain the search keys that describe the boundaries of the index
        // count, like "(event_type = 1 AND event_start > x.start_time)"
        if (searchKeySize > 0) {
            String start = explainKeys(asIndexed, m_searchKeyExpressions,
                    m_targetTableName, m_lookupType);
            usageInfo += "\n" + indent + " count matches from " + start;
        }
        if (endKeySize > 0) {
            String end = explainKeys(asIndexed, m_endKeyExpressions,
                    m_targetTableName, m_endType);
            usageInfo += "\n" + indent + " count matches to " + end;
        }
        if (m_skipNullPredicate != null) {
            String predicate = m_skipNullPredicate.explain(m_targetTableName);
            usageInfo += "\n" + indent + " discounting rows where " + predicate;
        }
        // Describe the table name and either a user-provided name of the index
        // or its user-specified role ("primary key").
        ...
        // Bring all the pieces together describing the index, how it is
        // scanned, and whatever extra filter processing is done to the result.
        retval += indexDescription;
        retval += usageInfo;
        return retval;
    }

    private static String explainKeys(String[] asIndexed,
            List<AbstractExpression> keyExpressions,
    ...
    @Override
    public void findAllExpressionsOfClass(
